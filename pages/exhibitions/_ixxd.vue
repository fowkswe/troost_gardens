  ...
  .exhibition_id
    section
      b-row
        ...
        b-col(cols='8')
          .hero
            b-img(fluid src='http://localhost:3000/uploads/work/images/9/IMG_4638.jpg')
            .caption 
    ...
    section
      h3 Exhibition Works 
      b-row
        b-col(v-for='work in exhibition.works' :key='work.id' cols='4')
          .work(@click='showWork(work)')
            .image(v-if='image(work)')
              nuxt-img(:src='image(work)')
            .info
              h5 {{work.title}}
...
export default
  components: { InstagramLogo, WorkModal }
  data: ->
    exhibition: null
    work: null
  methods:
    ...
    showWork: (work) ->
      ...
      @work = work
  asyncData: (context) ->
    # exhibition = await context.$axios.get('https://wp.docomomo-nytri.org//wp-json/wp/v2/posts?orderby=date&order=desc')
